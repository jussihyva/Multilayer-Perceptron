...
/*   Updated: 2021/12/13 11:40:38 by jkauppi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "multilayer_perceptron.h"

static const void	**layers_init(
							...
							const t_hyper_params *const hyper_params)
{
	...
	while (++i < NUM_OF_LAYERS)
		layers[i] = layer_init(i, layer_types[i], dataset, hyper_params);
	return (layers);
}
								...
								const t_hyper_params *const hyper_params)
{
	...
	neural_network->layers = layers_init(neural_network->layer_types, dataset,
			hyper_params);
	return (neural_network);
}

static void	layers_remove(const void **const *layers)
{
	size_t			i;
